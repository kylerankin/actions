...
sys.path.insert(0, str(Path(__file__).parent.parent / "bootc-build" / "generate-manifest-changelog"))
...
class TestLoadConfig:
    ...
    def test_production_config_uses_only_stable_target(self):
        loaded = load_config(str(Path(__file__).parent.parent / "bootc-build" / "generate-manifest-changelog" / "changelog_config.yaml"))
        assert loaded.targets == ["stable"]
