...
class TestComputePipelineHealth:
    ...
    def test_cancelled_runs_excluded_from_total(self):
        # A cancelled run never produced a build result (aborted by a push, tag
        # advance, or manual cancel), so it is excluded from the success rate
        # exactly like a skipped run. This is the projectbluefin/dakota #482 fix:
        # 6 success + 4 cancelled must read as 100% healthy, not 60% alert.
        success = [_make_run("success", minutes_ago=30) for _ in range(6)]
        cancelled = [_make_run("cancelled", minutes_ago=30) for _ in range(4)]
        result = compute_pipeline_health(success + cancelled, CUTOFF_1H_AGO)
        assert result["total"] == 6  # cancelled doesn't count
        assert result["success"] == 6
        assert result["rate_value"] == 100
        assert result["status"] == "healthy"
        assert result["failures_md"] == ""  # cancelled is not a failure to list

    def test_mixed_conclusions_rate_calculation(self):
        # 6 success, 2 failure, 2 cancelled = 8 counted (cancelled excluded),
        # 75% success -> alert. Only real build outcomes count against the rate.
        runs = (
