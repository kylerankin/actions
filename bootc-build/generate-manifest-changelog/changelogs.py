...
Examples:
  ...
  %(prog)s lts --pretty "Custom Version" --handwritten notes.txt
    ...
    parser.add_argument(
        "--output-file",
        help="Override output_file from config (path to write changelog.md)",
    )
    parser.add_argument(
        "--env-output-file",
        help="Override env_output_file from config (path to write output.env)",
    )
...
def main():
    ...
    try:
        ...
        if not args.dry_run:
            # Use CLI overrides or fall back to config defaults
            changelog_path = Path(args.output_file) if args.output_file else Path(config.defaults["output_file"])
            changelog_path.write_text(changelog, encoding="utf-8")
            logger.info(f"Changelog written to {changelog_path}")

            output_path = Path(args.env_output_file) if args.env_output_file else Path(config.defaults["env_output_file"])
