...
def file_uses_projectbluefin(path):
    ...
    callers, so they must not trip the thin-caller gate. Consumer repos copy
    such examples into their workflow headers, so flagging them would make the
    gate false-positive on exactly the repos #546 wants to enforce against.
    """
